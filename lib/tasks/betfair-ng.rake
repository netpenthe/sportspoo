...
namespace :betfair do
  ...
  task :update_ng => :environment do
    ...
    uri = URI.parse("#{endpoint}listMarketCatalogue/")
    #uri = URI.parse("#{endpoint}listEventTypes/")
    ...
    #request.body = "{\"filter\":{\"marketTypeCodes\":[\"MATCH_ODDS\"], \"marketCountries\":[\"USA\"]}, \"maxResults\":1000, \"marketProjection\":[\"EVENT\", \"COMPETITION\",\"EVENT_TYPE\"]}"
    #request.body = "{\"filter\":{\"marketTypeCodes\":[\"MATCH_ODDS\"], \"eventTypeIds\":[6423,7511,7522]}, \"maxResults\":1000, \"marketProjection\":[\"EVENT\", \"COMPETITION\",\"EVENT_TYPE\"]}"
    #request.body = "{\"filter\":{\"textQuery\":\"GB\"}}"
    #request.body = "{\"filter\":{\"competitionIds\":[\"7\"]}}"
    request.body = "{\"filter\":{\"marketTypeCodes\":[\"MATCH_ODDS\"]}, \"maxResults\":1000, \"marketProjection\":[\"EVENT\", \"COMPETITION\",\"EVENT_TYPE\"]}"
    #request.body = "{\"filter\":{}}"
    ...
    #australia = JSON.parse json_response
    australia = JSON.parse response.body

    australia.each do |event|
      sport = event["eventType"]["name"].gsub("Soccer","Football").gsub("Australian Rules","Football - Australian")
      league = event["competition"]["name"].gsub(" 2014","")
      event_name = event["event"]["name"]
      start_time = event["event"]["openDate"]
      evnt = {}
      evnt[:teams] = event_name.split(" v ").map{|t| {:name=>t, :id=>nil} } 
